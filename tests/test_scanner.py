...
class TestUvAuditParsing:
    def test_parses_vulnerabilities(self):
        output = """Found 1 known vulnerability and no adverse project statuses in 1 package
...
- GHSA-r9hx-vwmv-q579: pypa/setuptools vulnerable to Regular Expression Denial of Service (ReDoS)
  ...
  Advisory information: https://nvd.nist.gov/vuln/detail/CVE-2022-40897
"""
...
class TestScanProjectWithUpdates:
    ...
    def test_scan_deduplicates_vuln_and_update(self, scan_results_dir: Path):
        ...
        audit = subprocess.CompletedProcess(
            args=[],
            returncode=1,
            stdout="""Found 1 known vulnerability and no adverse project statuses in 1 package
...
class TestUvNativeScan:
    def test_uv_project_uses_uv_audit_locked_not_trivy_vulns(self, scan_results_dir: Path, tmp_path: Path):
    ...
    def test_uv_project_scans_secrets_when_enabled(self, scan_results_dir: Path, tmp_path: Path):
        ...
        with (
            patch("maintenance_man.scanner.subprocess.run", side_effect=[audit, trivy]) as run,
            patch("maintenance_man.scanner.get_outdated", return_value=[]),
        ):
            result = scan_project("test-proj", project)
