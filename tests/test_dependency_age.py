...
class TestFilterByAge:
    ...
    def test_filters_young_npm_package(self):
        ...
        two_days_ago = datetime(2026, 1, 28, tzinfo=timezone.utc).isoformat()
        npm_data = {"time": {"4.17.21": two_days_ago}}

        with patch(_PATCH_FETCH, return_value=npm_data):
    ...
    def test_keeps_old_npm_package(self):
        ...
        updates = [_make_update("lodash", "4.17.21")]
        thirty_days_ago = datetime(2025, 12, 31, tzinfo=timezone.utc).isoformat()
        npm_data = {"time": {"4.17.21": thirty_days_ago}}
    ...
    def test_keeps_update_on_registry_error(self):
        ...
        with patch(_PATCH_FETCH, side_effect=Exception("network error")):
            result = filter_by_age(updates, manager="bun", min_age_days=7)

        assert len(result) == 1
        assert result[0].published_date is None

    def test_pypi_lookup(self):
        """Test PyPI registry lookup for uv packages."""
        updates = [_make_update("requests", "2.31.0")]
        thirty_days_ago = "2025-12-31T00:00:00"

        pypi_data = {
            "releases": {
                "2.31.0": [{"upload_time_iso_8601": thirty_days_ago}],
            }
        }

        with patch(_PATCH_FETCH, return_value=pypi_data):
            with patch(_PATCH_NOW, return_value=_FROZEN_NOW):
