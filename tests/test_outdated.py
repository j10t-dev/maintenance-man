...
class TestUvOutdated:
    def test_syncs_environment_before_checking_outdated(self, tmp_path):
        ...
        sync_completed = subprocess.CompletedProcess(args=[], returncode=0, stdout="", stderr="")
        list_completed = subprocess.CompletedProcess(args=[], returncode=0, stdout="[]", stderr="")
        ...
        assert run.call_args_list[1].args[0][:5] == ["uv", "pip", "list", "--outdated", "--format"]

    def test_sync_failure_raises(self, tmp_path):
        pyproject = tmp_path / "pyproject.toml"
        pyproject.write_text('[project]\ndependencies = ["requests==2.31.0"]\n')
        failed = subprocess.CompletedProcess(args=[], returncode=1, stdout="", stderr="lock stale")
        project = ProjectConfig(path=tmp_path, package_manager="uv")
    ...
    def test_parses_json_output(self, tmp_path):
        ...
        with patch("maintenance_man.outdated.subprocess.run", side_effect=[sync_completed, list_completed]):
            updates = uv_outdated(project)
    ...
    def test_command_failure_raises(self, tmp_path):
        ...
        with patch("maintenance_man.outdated.subprocess.run", side_effect=[sync_completed, list_failed]):
