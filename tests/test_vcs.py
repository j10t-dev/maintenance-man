...
# ---------------------------------------------------------------------------
# jj label/bookmark/change helpers
# ---------------------------------------------------------------------------
...
class TestEnsureMainBookmark:
    ...
    @patch("maintenance_man.vcs.bookmark_exists", side_effect=[False, True])
    @patch("maintenance_man.vcs._run")
    def test_creates_main_from_origin(
        self, mock_run: MagicMock, _mock_exists: MagicMock, tmp_path: Path
    ):
        mock_run.return_value = _completed()
        assert ensure_main_bookmark(tmp_path) is True
        mock_run.assert_called_once_with(
            ["jj", "bookmark", "create", "main", "-r", "main@origin"], tmp_path
        )

    @patch("maintenance_man.vcs.bookmark_exists", side_effect=[False, False])
    def test_missing_main_and_origin_is_false(
        self, _mock_exists: MagicMock, tmp_path: Path
    ):
